...
time_period = ''                #label time period (ie. AM, MD, PM)
direction = ''                  #label direction (ie. WB, EB, NB, SB) of street (for charts and gpx files)
route_reverse = False           #set False for same direction intersection coordinates are listed and True for opposite direction of intersection coordinates.
run_num = 1                     #set to number of run for GPX file


#load file and parse for one direction
gpx_file = open('data/'+street_name+'-'+time_period+'-'+str(run_num)+'.gpx','r')
...
intersectionDF.loc[0] = [] + [] #1 - Street name
intersectionDF.loc[1] = [] + [] #2 - Street Name
intersectionDF.loc[2] = [] + [] #3 - Street Name
